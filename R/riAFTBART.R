...
#'
#' The function implements the random effect accelerated failure time BART (riAFT-BART) for causal inference with multiple treatments and clustered survival outcomes.
...
#' res_ate <- riAFTBART(M.burnin = 50, M.keep = 50, M.thin = 1, status = status,
#'                       y = Y, trt = trt.train,
#'                       x = cbind(x1,x2),
...
riAFTBART <- function(M.burnin, M.keep, M.thin = 1, status, y, x, trt, cluster.id, verbose = FALSE, estimand = "ATE", reference_trt = NULL){
  ...
  if (estimand == "ATE") {
    ...
    for (i in 1:(n_trt-1)){
      for (j in (i + 1):n_trt){
        result <- c(result, (eval(parse(text = (paste0("logT", i, j, "_est"))))))
        names(result)[[counter]] <- paste0("logT", i, j)
        counter <- counter +1
      }
    }
    return(result)
    ...
    counter <- 1
      for (j in 1:length(trt_indicator_no_reference)){
        result <- c(result, (eval(parse(text = (paste0("logT",reference_trt, trt_indicator_no_reference[j], "_est"))))))
        names(result)[[counter]] <- paste0("logT", reference_trt, trt_indicator_no_reference[j])
        counter <- counter +1
      }
    return(result)
  }
